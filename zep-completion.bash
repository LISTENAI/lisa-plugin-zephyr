...
__comp_zep()
{
	...
	local cmds=(${plugin_cmds[*]} ${builtin_cmds[*]} ${zephyr_ext_cmds[*]})
	...
	local command='lisa' command_pos=0
	local counter=1
	while [ "$counter" -lt "$cword" ]; do
		case "${words[$counter]}" in
			$(__west_to_extglob "$global_args_opts") )
	...
	local completions_func=__comp_zep_${command//-/_}
