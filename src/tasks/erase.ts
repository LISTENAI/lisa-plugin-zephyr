...
export default ({ application, cmd }: LisaType) => {
  job('erase', {
    ...
    async task(ctx, task) {
      ...
      const runnerYml = await buildDir.runnerYml();
      const runner: string = args['runner'] || runnerYml[RUNNER_KEY];
      ...
      const runnerArgs: {[key: string]: string} = {}
      if (runnerYml?.args?.[runner]) {
        runnerYml?.args?.[runner].forEach((item: string) => {
          const match = item.match(/^--(\w+)=(\w+)/)
          if (match) {
            runnerArgs[match[1]] = match[2];
          }
        })
      }
      const commandArgs = [];
