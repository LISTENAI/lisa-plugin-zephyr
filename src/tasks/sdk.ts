  ...
  job("sdk", {
    ...
    async task(_ctx, task) {
      ...
      if (args === "set") {
        ...
        if (addArgs.length > 0) {
          await cmd("lisa", ["zep", "use-sdk"].concat(addArgs), { stdio: 'inherit' });
        } else {
          //lisa zep sdk set
          //1 下载sdk zip包 2 解压 3 use-sdk
          await cmd("lisa", ["zep", "use-sdk", "--clear"], { stdio: 'inherit' });
          let mr = ''
          ...
          // E: \lisa - zep - sdk - set\.listenai\csk- sdk\zstd\zstd.exe - d  E: \lisa - zep - sdk - set\.listenai\csk - sdk\v1.0.2 - beta.1.tar.zst
          await exec(join(zstd, 'zstd.exe'), ['-d', sdkZSTPath])
          // "D:\Program Files\7-Zip\7z.exe"  x  E: \lisa - zep - sdk - set\.listenai\csk- sdk\v1.0.2 - beta.1.tar
          await exec(join(unzipApp, '7z.exe'), ['x', sdkZipPath, `-o${sdkPath}`])
      ...
      if (args === "use") {
        ...
        tags.forEach((item: any) => {
          if (item.name === version) { item.name = item.name + '(current)' }
        });
        ...
        await cmd("lisa", ["zep", "use-sdk", "--mr", tagName], { stdio: 'inherit' });
      }
      const sdk = await get("sdk");
      const version = sdk ? await sdkTag(sdk) : null;
